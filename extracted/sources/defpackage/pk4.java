...
public final class pk4 {
    ...
    public static final void c(float[] fArr, qb5 qb5Var) {
        if (fArr.length < 16) {
            return;
        }
        float f = fArr[0];
        ...
        qb5Var.b = Math.min(f19, Math.min(f26, Math.min(f32, f37)));
        qb5Var.c = Math.min(f22, Math.min(f28, Math.min(f34, f38)));
        qb5Var.d = Math.max(f19, Math.max(f26, Math.max(f32, f37)));
        qb5Var.e = Math.max(f22, Math.max(f28, Math.max(f34, f38)));
    }
